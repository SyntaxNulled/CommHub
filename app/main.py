...
from app.automation.scheduler import start_scheduler, stop_scheduler, add_cron_job, scheduler as _scheduler
...
async def finalize_pending_sends():
    ...
    async with async_session_factory() as session:
        ...
        result = await session.execute(
            select(Email).where(Email.send_at.isnot(None), Email.send_at <= now)
        )
...
async def lifespan(app: FastAPI):
    ...
    _scheduler.add_job(
        ...
        id="finalize_pending_sends",
        name="finalize_pending_sends",
